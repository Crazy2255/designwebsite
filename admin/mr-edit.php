...
// Fetch marble
$stmt = $conn->prepare("SELECT * FROM marbles WHERE id = ?");
...
if (!$marble) {
    $_SESSION['error'] = "Marble not found!";
    header("Location: mr-index.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($name)) {
        ...
        $image = $marble['image']; // Keep existing image by default
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
            ...
            if (!in_array(strtolower($filetype), $allowed)) {
                ...
                $target = 'uploads/marbles/' . $new_image;
                ...
                if (move_uploaded_file($_FILES['image']['tmp_name'], $target)) {
                    ...
                    if (!empty($marble['image'])) {
                        $old_image = 'uploads/marbles/' . $marble['image'];
        ...
        if (!isset($_SESSION['error'])) {
            // Update marble
            $stmt = $conn->prepare("UPDATE marbles SET name = ?, category_id = ?, image = ?, status = ? WHERE id = ?");
            $stmt->bind_param("sissi", $name, $category_id, $image, $status, $id);
            
            if ($stmt->execute()) {
                $_SESSION['success'] = "Marble updated successfully!";
                header("Location: mr-index.php");
                exit();
            } else {
                $_SESSION['error'] = "Failed to update marble!";
            }
        }
    }
}
            ...
            <div class="container-fluid px-4">
                ...
                <nav aria-label="breadcrumb" class="mt-4">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Edit Marble</li>
                    </ol>
                </nav>

                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h1 class="h3 mb-0">Edit Marble</h1>
                        ...
                        <form action="" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="col-md-8">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" id="name" name="name" 
                                               value="<?php echo htmlspecialchars($marble['name']); ?>" 
                                               required>
                                    </div>
                                        ...
                                        <select class="form-select" id="category_id" name="category_id" required>
                                            ...
                                            <?php foreach ($categories as $category): ?>
                                                <option value="<?php echo $category['id']; ?>" 
                                                    <?php echo $marble['category_id'] == $category['id'] ? 'selected' : ''; ?>>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="form-check form-switch">
                                            <input class="form-check-input" type="checkbox" id="status" name="status" 
                                                   value="active" <?php echo $marble['status'] == 'active' ? 'checked' : ''; ?>>
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="text-center my-3">
                                                <?php if (!empty($marble['image']) && file_exists('uploads/marbles/' . $marble['image'])): ?>
                                                    <img id="imagePreview" src="uploads/marbles/<?php echo $marble['image']; ?>" 
                            ...
                            <div class="mt-4 border-top pt-3">
                                <button type="submit" class="btn btn-primary">
                                    <i class="fas fa-save me-2"></i>Update Marble
                                </button>
