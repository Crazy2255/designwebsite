...
$selected_colors = [];
$stmt = $conn->prepare("SELECT color_id FROM product_colors WHERE product_id = ?");
$stmt->bind_param("i", $id);
$stmt->execute();
$result = $stmt->get_result();
while ($row = $result->fetch_assoc()) {
...
$result = $stmt->get_result();
while ($row = $result->fetch_assoc()) {
    $selected_materials[] = $row['material_id'];
}

$selected_marbles = [];
$stmt = $conn->prepare("SELECT marble_id FROM product_marbles WHERE product_id = ?");
$stmt->bind_param("i", $id);
$stmt->execute();
$result = $stmt->get_result();
while ($row = $result->fetch_assoc()) {
    $selected_marbles[] = $row['marble_id'];
}

$selected_wooden = [];
$stmt = $conn->prepare("SELECT wooden_id FROM product_wooden WHERE product_id = ?");
$stmt->bind_param("i", $id);
$stmt->execute();
$result = $stmt->get_result();
while ($row = $result->fetch_assoc()) {
    $selected_wooden[] = $row['wooden_id'];
}

// Fetch existing images
$product_images = [];
$stmt = $conn->prepare("SELECT id, image, sort_order FROM product_images WHERE product_id = ? ORDER BY sort_order");
$stmt->bind_param("i", $id);
$stmt->execute();
$result = $stmt->get_result();
while ($row = $result->fetch_assoc()) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($name)) {
        ...
        if ($stmt->get_result()->num_rows > 0) {
            ...
            try {
                // Update product
                $stmt = $conn->prepare("UPDATE products SET name = ?, category_id = ?, subcategory_id = ?, description = ?, sku = ?, price = ?, quantity = ?, length = ?, width = ?, height = ?, weight = ?, status = ? WHERE id = ?");
                $stmt->bind_param("siissiiddddsi", $name, $category_id, $subcategory_id, $description, $sku, $price, $quantity, $length, $width, $height, $weight, $status, $id);
                $stmt->execute();
                        ...
                        if ($_FILES['images']['error'][$key] == 0) {
                            ...
                            if (in_array(strtolower($filetype), $allowed)) {
                                ...
                                if (move_uploaded_file($tmp_name, $filepath)) {
                                    // Insert image record
                                    $sort_order = count($product_images) + $key;
                                    $stmt = $conn->prepare("INSERT INTO product_images (product_id, image, sort_order) VALUES (?, ?, ?)");
                                    $stmt->bind_param("isi", $id, $new_filename, $sort_order);
                                    $stmt->execute();
                                }
                            }
                        }
                ...
                foreach ($relationship_data as $rel) {
                    ...
                    $stmt = $conn->prepare("DELETE FROM {$rel['table']} WHERE product_id = ?");
                    $stmt->bind_param("i", $id);
                    $stmt->execute();
                                ...
                                <div class="col-md-12">
                                                    ...
                                                    <?php foreach ($materials as $material): ?>
                                                        <div class="form-check">
                                                            <input class="form-check-input" type="checkbox" name="materials[]" 
                                                                   ...
                                                                   <?php echo in_array($material['id'], $selected_materials) ? 'checked' : ''; ?>>
                                    ...
                                    <div class="row">
                                                    ...
                                                    <?php foreach ($marbles as $marble): ?>
                                                        <div class="form-check">
                                                            <input class="form-check-input" type="checkbox" name="marbles[]" 
                                                                   ...
                                                                   <?php echo in_array($marble['id'], $selected_marbles) ? 'checked' : ''; ?>>
                                        ...
                                        <div class="col-md-6">
                                            <div class="card mb-3">
                                                <div class="card-header">
                                                    <h5 class="card-title mb-0">Wooden Items</h5>
                                                </div>
                                                <div class="card-body" style="max-height: 200px; overflow-y: auto;">
                                                    <?php foreach ($wooden as $wooden): ?>
                                                        <div class="form-check">
                                                            <input class="form-check-input" type="checkbox" name="wooden[]" 
                                                                   value="<?php echo $wooden['id']; ?>" id="wooden<?php echo $wooden['id']; ?>"
                                                                   <?php echo in_array($wooden['id'], $selected_wooden) ? 'checked' : ''; ?>>
                                                            <label class="form-check-label" for="wooden<?php echo $wooden['id']; ?>">
                                                ...
                                                <?php foreach ($product_images as $image): ?>
                                                    <div class="col-6">
                                                        <div class="position-relative">
                                                            <img src="uploads/products/<?php echo $image['image']; ?>" 
                                                                 alt="Product Image" 
...
const currentSubcategory = <?php echo $product['subcategory_id'] ? $product['subcategory_id'] : 'null'; ?>;
